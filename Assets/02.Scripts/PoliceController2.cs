...
public class PoliceController2 : MonoBehaviour
{
    public enum State
    {
        WAIT,
    ...
    public void fDetected()
    {
        ...
        isCarriable = false;
        gameObject.GetComponent<PoliceController2>().isDetected = true;
    }
        ...
        for (int i = 0; i < corpses.Length; i++)
        {
            ...
            if (Vector3.Angle(transform.forward, dirToCorpse) <= 90f)
            {
                ...
                if (!Physics.Raycast(transform.position, dirToCorpse, distToCorpse, layerMask))
                {
                    ...
                    if (tempCorpse.CompareTag("Police")) tempCorpse.GetComponent<PoliceController2>().fDetected();
